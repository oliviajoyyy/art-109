...
const boxGeometry = new THREE.BoxGeometry(24, 8, 24, 24, 4, 24);
const boxMaterial = new THREE.ShaderMaterial({
    wireframe: true,
    uniforms: uniformData,
    vertexShader: `
      ...
      void main()	{
        ...
        result = vec4(
          10.0*cos(position.x/10.0 + u_time) * 1.5,
          5.0*sin(position.z/3.0 + u_time) + position.y,
          position.z * 2.0,
          1.5
        );
    ...
    fragmentShader: `
      ...
      void main() {
        if (pos.x >= -0.5 && pos.x <= 0.5) {
          gl_FragColor = vec4(1.0, 1.0, 1.0, 1.0); // set to white
          
        } else if (pos.x >= 0.0) {
          gl_FragColor = vec4(0.7, 0.0, 0.0, 1.0); // set to red if x >= 0
          // gl_FragColor = vec4(abs(sin(u_time)), 0.0, 0.0, 1.0);

          if (pos.y > 0.0) { // lower half
            gl_FragColor = vec4(0.5, 0.5, 0.0, 1.0); // set to orange
          }

        } else {
          gl_FragColor = vec4(0.0, 0.7, 0.5, 1.0); // set to green
          // gl_FragColor = vec4(0.0, abs(cos(u_time)), 0.0, 1.0);

          if (pos.y > 0.0) { // upper half
            gl_FragColor = vec4(0.0, 0.0, 0.7, 1.0); // set to blue
          }
        }
        ...
        if (pos.x + pos.z >= -0.5 && pos.x + pos.z <= 0.5) {
            gl_FragColor = vec4(1.0, 1.0, 1.0, 1.0); // set to white
        }
